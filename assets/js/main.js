...
function initializeLoading() {
    ...
    const friendlyMessages = [
        'Preparando um mundo de descobertas...',
        'Organizando os brinquedos...',
        'Preparando as atividades...',
        'Quase pronto para brincar!',
    ...
    function rotateMessages() {
        updateStatus(friendlyMessages[messageIndex]);
        messageIndex = (messageIndex + 1) % friendlyMessages.length;
    }
    
    rotateMessages();
    const messageInterval = setInterval(rotateMessages, 1500);
    
    function checkDependencies() {
        const checkInterval = setInterval(() => {
            if (window.dependenciesLoaded) {
                const deps = window.dependenciesLoaded;
                let loadedCount = 0;
                let totalDeps = 3;
                
                if (deps.fontAwesome) loadedCount++;
                if (deps.googleFonts) loadedCount++;
                if (deps.aos) loadedCount++;
                
                if (loadedCount === totalDeps || (Date.now() - startTime) > 6000) {
    ...
    setTimeout(() => {
        console.warn('Tempo limite de carregamento atingido, forçando exibição da página');
        imagesLoaded = true;
        dependenciesLoaded = true;
        minimumTimeElapsed = true;
    ...
    function checkIfAllLoaded() {
        if (imagesLoaded && dependenciesLoaded && minimumTimeElapsed) {
            clearInterval(messageInterval);
            updateStatus('Pronto!');
    ...
    function hideLoadingScreen() {
        ...
        if (typeof AOS !== 'undefined') {
            try {
                ...
                console.warn('Erro ao inicializar AOS:', e);
            }
        }
...
function forceSkipLoading() {
    console.log('Carregamento forçado pelo usuário');
    const loadingScreen = document.getElementById('loading-screen');
